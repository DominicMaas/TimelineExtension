import { GetRemoteDevicesMessage, LoginMessage, LogoutMessage, OpenOnRemoteDeviceMessage } from "./common/messages/messages";
...
function updateMinimumTimeOnPage() {
    (<HTMLInputElement> document.getElementById('setting-timeout')).value = minimumTimeOnPage.toString();
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    attachClickEvent('view-devices', () => {
        ...
        Message.sendMessageWithResult<any>(new GetRemoteDevicesMessage(), data => {
            ...
            data.payload.forEach(device => {
                // Get the description
                var description = device.Model == null ? device.Platform + ' Device' : device.Model;
    ...
    document.getElementById('setting-timeout').addEventListener('change', function() {
        setMinimumTimeOnPage(Number((<HTMLInputElement> document.getElementById('setting-timeout')).value));
    });
});
